...
class Run:
    ...
    def __init__(self, com):
        ...
        if txt.find("2015"):
            # 按点按钮开心下
            c.mouse_move(self.xy_offset(rect, position.main_btn3))
            c.mouse_press("LEFT")
            c.mouse_free()
            time.sleep(0.2)
            c.keyboard("ESC", 50)

            c.mouse_move(self.xy_offset(rect, position.main_btn2))
            c.mouse_press("LEFT")
            c.mouse_free()
            time.sleep(0.2)
            c.mouse_move(self.xy_offset(rect, position.main_btn2_close))
            c.mouse_press("LEFT")
            c.mouse_free()

            # 甩一波窗口
            start = self.xy_offset(rect, position.title)
            c.mouse_move(start)
            c.mouse_press("LEFT")
            limit = 3
            while limit > 0:
            ...
            # 退出
            c.mouse_move(self.xy_offset(rect, position.main_btn4))
            c.mouse_press("LEFT")
            c.mouse_free()
